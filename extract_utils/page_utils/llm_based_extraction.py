...
### OCCUPATION
    ...
    ### REASON FOR G325A STUFF 
    def inference_llm_g325_reason(self, url, prompt, text_postprocess_fn=None):
        """ 
        Extract occupation with an LLM.
        """
        prompt = (
            "In the form, identify the checkbox status for the field without hallucinating."
            "'THIS FORM IS SUBMITTED IN CONNECTION WITH APPLICATION FOR'. "
            "Is it checked or unchecked?"
            "If checked, which value?"
            "PLEASE DONT HALLUCINATE"
        )
        # Retrieve the image from the URL
        try:
            messages = [
    ...
    def run_inference_on_csv_g325a_reason(self, prompt, text_postprocess_fn, col_name,col_datatype='object', init_val=None, batch_size=20):
        ''' 
        very general script to run vllm inference for a given prompt and store the results into a csv on a particular column
